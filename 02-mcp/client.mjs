...
// ── ① 连接 MCP server（把 server.mjs 当子进程拉起来，走 stdio）────────────────
...
async function runAgent(userQuestion) {
  ...
  for (let step = 0; step < 5; step++) {
    ...
    const msg = await chat({ messages, tools: llmTools });
    messages.push(msg);
