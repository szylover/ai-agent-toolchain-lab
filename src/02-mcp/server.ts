// ──────────────────────────────────────────────────────────────────────────
// 第 2 课 · 服务端：一个【真正的 MCP Server】（官方 @modelcontextprotocol/sdk）
//
// 重点（把你之前的疑问彻底坐实）：
...
//
// 你不用单独启动它——client.ts 会把它当子进程拉起来（stdio transport）。
// ──────────────────────────────────────────────────────────────────────────
...
const log = (...a: unknown[]) => console.error("  [MCP server]", ...a);

const server = new McpServer({ name: "weather-shop-server", version: "1.0.0" });

// 工具 1：查天气。注意这里收到的是【结构化参数】，不是一句自然语言。
// zod schema 会被 SDK 自动转成 MCP 的 inputSchema（JSON Schema）。
server.tool(
  "getWeather",
  "查询某城市的实时天气。",
