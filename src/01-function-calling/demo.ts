...
//
// 跑法：npm run fc      （配了 .env 用真模型，否则 mock）
// ──────────────────────────────────────────────────────────────────────────
...
function searchProduct({ keyword }: { keyword: string }) {
  return { keyword, results: [`${keyword} Pro`, `${keyword} Mini`], count: 2 };
}

// 把本地函数登记成「工具表」，key 必须和下面 tools 里的 name 对得上。
const toolImpls: Record<string, (args: any) => unknown> = { getWeather, searchProduct };
...
async function runAgent(userQuestion: string): Promise<void> {
  ...
  for (let step = 0; step < 5; step++) {
    ...
    for (const call of msg.tool_calls) {
      ...
      const result = fn ? fn(args) : { error: "no such tool" };
